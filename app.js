...
(function () {
  ...
  function cacheEls() {
    [
      ...
      "toast", "startOverlay", "startBtn", "startStatus"
  ...
  function updateAppleConnectUI() {
    ...
    if (state.appleConnected) {
      els.appleConnectBtn.textContent = "連携済み ✓ 自動追加ON";
      els.appleConnectBtn.classList.add("connected");
      els.appleConnectNote.textContent = "左右スワイプ(いいね)した曲は Apple Music の「Disco Swipe」プレイリストに自動追加されます。";
    } else {
      els.appleConnectBtn.textContent = "Apple Musicと連携";
      els.appleConnectBtn.classList.remove("connected");
      els.appleConnectNote.textContent = "連携すると、左右スワイプ(いいね)した曲が自動で Apple Music のプレイリストに追加されます。（要 Apple Music サブスク）";
    }
  }
